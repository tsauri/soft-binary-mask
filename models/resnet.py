...
def identity_func(n_in_planes, n_out_planes, stride):
  if stride == 2 and n_in_planes != n_out_planes:
    def identity(x): return _downsample_and_increase_planes(x, n_out_planes)
  elif stride == 2:
    def identity(x): return_down_sample(x)
  elif n_in_planes != n_out_planes:
    def identity(x): return _increase_planes(x, n_out_planes)
  else:
    return identity
...
class BasicBlock(nn.Module):
  ...
  def __init__(self, n_in_planes, n_out_planes, stride=1):
    ...
    self.stride = stride
    self.n_in_planes = n_in_planes
    self.n_out_planes = n_out_planes

    # self.identity = identity_func(n_in_planes, n_out_planes, stride)
    
  def identity(self,x):
    stride = self.stride
    n_in_planes = self.n_in_planes
    n_out_planes = self.n_out_planes
    if stride == 2 and n_in_planes != n_out_planes:
      return _downsample_and_increase_planes(x, n_out_planes)
    elif stride == 2:
      return_down_sample(x)
    elif n_in_planes != n_out_planes:
      return _increase_planes(x, n_out_planes)
    else:
      return x
  def forward(self, x):
    out = self.block(x)
...
class ResNet(nn.Module):
  ...
  def __init__(self, block, 
    ...
    self.fc = nn.Linear(n_output_planes[3] * block.expansion, n_classes, True)
...
def resnet(dataset='cifar100', depth=18, widen_factor=0, dropout_rate=0):
  n_classes = 100 if dataset == 'cifar100' else 10
  n_output_planes = [64, 128, 256, 512]
  if depth == 10:
    return ResNet10(n_classes=n_classes, n_output_planes=n_output_planes)
  elif depth == 18:
    return ResNet18(n_classes=n_classes, n_output_planes=n_output_planes)
  elif depth == 34:
    return ResNet34(n_classes=n_classes, n_output_planes=n_output_planes)
